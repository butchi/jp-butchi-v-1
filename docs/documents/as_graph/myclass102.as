...
//	this.graphMC = this.mc.createEmptyMovieClip("graph_mc",2); ->plotに移動@101
	...
	//軸矢印の描画(移動)
	this.mc._parent.xarrow._x=this.mc._x+this.dispx(this.right);
	this.mc._parent.xarrow._y=this.mc._y+this.dispy(0);
	this.mc._parent.yarrow._x=this.mc._x+this.dispx(0);
	this.mc._parent.yarrow._y=this.mc._y+this.dispy(this.top);
	//全体にマスクをかける
	this.maskMC = this.mc.duplicateMovieClip("mask_mc", 2);  //4でなく2でも大丈夫だよね？@101
	g1.mc.setMask(this.maskMC);
};
//グラフを描画する 多少改良@101 不連続のための処理@102
